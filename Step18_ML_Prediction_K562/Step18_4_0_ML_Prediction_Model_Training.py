...
from matplotlib.colors import LinearSegmentedColormap  # <--- MODIFICATION 1: Import added
...
# --- Configuration ---
# Note: Paths preserved for K562 dataset
POSITIVE_SET_PATH = "../../public/multiomics/Step18_ML_Prediction_K562/hg38_bin_positive_histone.csv"
NEGATIVE_SET_PATH = "../../public/multiomics/Step18_ML_Prediction_K562/hg38_bin_negative_histone.csv"
OUTPUT_DIR = "../../public/multiomics/Step18_ML_Prediction_K562"
...
# --- NEW: Choose decision threshold on validation to maximize F1 ---
prec, rec, thresh = precision_recall_curve(y_val, val_preds_proba)
# Avoid division by zero for F1 at endpoints
f1s = (2 * prec * rec) / np.clip(prec + rec, 1e-9, None)
# precision_recall_curve returns an extra point for precision/recall with no threshold
valid_idx = np.arange(len(thresh))  # thresholds align with all but last PR point
...
# 4. Feature Importance Scores (model-centric)
...
# A) SHAP explanations (global + local)
if HAVE_SHAP:
    print("\nSHAP: Computing SHAP values for test set...")
    # TreeExplainer supports XGBoost tree models efficiently
    explainer = shap.TreeExplainer(model)  # model is xgb.Booster
    shap_values = explainer.shap_values(X_test)

    # Save SHAP values for the test set
    ...
    # -------------------------------------------------------------------------
    # MODIFICATION 2: Global SHAP summary with custom Top 5% Red / 95% Blue map
    # -------------------------------------------------------------------------
    
    # 1. Define custom colormap: Blue (0-95%) -> Red (95-100%)
    colors = [
        (0.00, "blue"),  # Start with Blue
        (0.95, "blue"),  # Stay Blue until 95%
        (0.95, "red"),   # Switch to Red instantly at 95%
        (1.00, "red")    # End with Red
    ]
    cmap_top5 = LinearSegmentedColormap.from_list("top5_red", colors)

    # 2. Rank transform the data (percentiles)
    # This ensures that exactly the top 5% of *points* are red, regardless of value distribution.
    X_test_percentile = X_test.rank(pct=True)

    shap.summary_plot(
        shap_values, 
        X_test_percentile,     # Use percentile data for coloring
        feature_names=features, 
        show=False,
        cmap=cmap_top5         # Apply custom 2-tone colormap
    )
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "shap_summary_beeswarm.jpg"), format='jpg', dpi=300, bbox_inches='tight')
    plt.close()
    print("- SHAP summary beeswarm saved (with Top 5% Highlight).")
    ...
    # ------------------------------------------------------------------
    # SHAP dependence plots with global ranges and original palette
    # ------------------------------------------------------------------
    from shap.plots import colors as shap_colors  # SHAP's plotting colors

    # Compute global min/max for every feature over the FULL combined dataset
    ...
    def plot_shap_dependence_with_global_colors(
        ...
        """
        Custom SHAP dependence-style scatter plot with:
          - fixed global color scale per interaction feature
          - SHAP's original red-blue color palette
          - low color values plotted first, high color values plotted last
        """
        if cmap is None:
            cmap = shap_colors.red_blue  # SHAP's default dependence colormap
        ...
        # Sort by color value so low-valued points are drawn first, high last
    ...
    # SHAP dependence plots for ALL feature combinations,
    # using the custom helper to enforce global color ranges
    ...
    try:
        ...
        for i, main_feat in enumerate(features):
            for j, interact_feat in enumerate(features):
                ...
                plot_shap_dependence_with_global_colors(
                    ...
                    cmap=shap_colors.red_blue,  # original SHAP palette
                    dot_size=16,
                    alpha=0.8,
                )
                ...
                plt.savefig(
                    os.path.join(
                        OUTPUT_DIR,
                        f"shap_dependence_{main_feat}__{interact_feat}.jpg",
                    ),
                    format="jpg",
                    dpi=300,
                    bbox_inches="tight",
        ...
        print(f"- Saved {count} SHAP dependence plots (all ordered combinations).")

    except Exception as e:
        print(f"Skipping SHAP dependence plots due to: {e}")

# B) Single-tree visualization (structure-level interpretability)
try:
    # Plot the first tree
...
# C) Lightweight Partial Dependence (manual, model-agnostic)
# For each feature: vary it over its empirical grid, hold others fixed, average predicted probability
