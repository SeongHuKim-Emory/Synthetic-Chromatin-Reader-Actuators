...
# - *** CUSTOM REVISION: Filters loaded features to remove '_025' entries ***
...
import optuna # <-- Keep import in case user re-enables it, though not used now
...
# --- CHANGED: Per user request, updated output directory ---
OUTPUT_BASE_DIR = "../../public/Multiomics/Step19_ML_UpDEG_Prediction_Final_Model_without_025"
# --- END CHANGE ---
...
# --- MODIFICATION: Added '_without_025_features' suffix to all output filenames ---
MODEL_BIN_PATH = os.path.join(OUTPUT_BASE_DIR, "UpDEG_Prediction_trained_model_without_025_features.bin")
MODEL_JSON_PATH = os.path.join(OUTPUT_BASE_DIR, "UpDEG_Prediction_trained_model_without_025_features.json")
TEST_PREDICTIONS_PATH = os.path.join(OUTPUT_BASE_DIR, "test_predictions_without_025_features.csv")
TEST_METRICS_PATH = os.path.join(OUTPUT_BASE_DIR, "test_metrics_without_025_features.csv")
TEST_CONFUSION_PATH = os.path.join(OUTPUT_BASE_DIR, "test_confusion_matrix_without_025_features.csv")
FEATURE_IMPORTANCE_PATH = os.path.join(OUTPUT_BASE_DIR, "feature_importance_without_025_features.csv")

# --- MODIFIED FILE PATHS (to OUTPUT_BASE_DIR) ---
# This file is no longer loaded, but written to by this script, so it's an OUTPUT
RFE_STEP1_FEATURES_PATH = os.path.join(OUTPUT_BASE_DIR, "selected_top_1000_features_rfe_without_025_features.json")
SELECTED_FEATURES_PATH = os.path.join(OUTPUT_BASE_DIR, "selected_final_features_without_025_features.json")

# --- REVISION: Optuna tuning paths removed ---
# BEST_PARAMS_PATH = os.path.join(OUTPUT_BASE_DIR, "best_model_params.json")
# TUNING_TRIALS_PATH = os.path.join(OUTPUT_BASE_DIR, "tuning_trials.csv")

# --- NEW FILE PATHS FOR PLOTS (to OUTPUT_BASE_DIR) ---
AUC_CURVE_PATH = os.path.join(OUTPUT_BASE_DIR, "UpDEG_Prediction_AUC_curve_without_025_features.jpg")
AUCPR_CURVE_PATH = os.path.join(OUTPUT_BASE_DIR, "UpDEG_Prediction_AUCPR_curve_without_025_features.jpg")
...
try:
    with open(PRESELECTED_FEATURES_JSON_PATH, "r") as f:
        STEP1_FEATURE_COLS_ORIGINAL = json.load(f) # Load into temp variable
    print(f"  Loaded {len(STEP1_FEATURE_COLS_ORIGINAL)} features.")
...
# --- NEW: Filter out features ending in _025 (Per User Request) ---
print(f"\n  NEW: Filtering out features ending in '_025'...")
STEP1_FEATURE_COLS = [
    f for f in STEP1_FEATURE_COLS_ORIGINAL if not f.endswith("_025")
]
num_removed = len(STEP1_FEATURE_COLS_ORIGINAL) - len(STEP1_FEATURE_COLS)
del STEP1_FEATURE_COLS_ORIGINAL # Free memory
print(f"  Removed {num_removed} features ending in '_025'.")
print(f"  Proceeding with {len(STEP1_FEATURE_COLS)} features.\n")
# --- END NEW ---


# 2. Save a copy of this *filtered* feature list to the new output directory
#    (This is what the original script did in Stage 1, Step 5)
print(f"  2. Saving a copy of this *filtered* feature list to {RFE_STEP1_FEATURES_PATH}...")
try:
    with open(RFE_STEP1_FEATURES_PATH, "w") as f:
        json.dump(STEP1_FEATURE_COLS, f, indent=4) # Saves the filtered list
except Exception as e:
    print(f"  WARNING: Could not save feature list copy. Error: {e}")


# 3. Validate loaded *filtered* features against the full dataset columns
print(f"  3. Validating loaded *filtered* features against {len(FEATURE_COLS_FULL)} columns from dataset...")
...
# Fit on the (filtered) 1000-feature *combined* training data
...
# 6. Export the trained model object
print(f"\n  6. Saving trained model (binary) to {MODEL_BIN_PATH}...")
