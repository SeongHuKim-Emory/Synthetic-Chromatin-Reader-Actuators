# Python 3.12.3
# UpDEG vs Non-DEG classifier with 2-Stage RFE + GridSearch
...
# - *** STAGE 2: Exhaustive Grid Search (finds best XGB params AND best n_features for RFE Step 2) ***
...
#
# You must install it first: pip install polars xgboost scikit-learn pandas scipy matplotlib
# ---
...
import threading
import pickle
import itertools # <-- NEW IMPORT for Grid Search
...
if IS_CUDA_AVAILABLE:
    ...
    # Parameters for GPU execution inside Grid Search trials
    GRIDSEARCH_DEVICE_PARAMS = {
        "tree_method": "hist",
        "device": "cuda"
    }
else:
    ...
    # Fallback for Grid Search trials
    GRIDSEARCH_DEVICE_PARAMS = {
...
# New paths for tuning
BEST_PARAMS_PATH = os.path.join(BASE_DIR, "best_model_params.json")
TUNING_TRIALS_PATH = os.path.join(BASE_DIR, "tuning_trials_grid_search.csv") # <-- This will store all Grid Search results
...
RANDOM_STATE = 42
# N_TRIALS = 100  # <-- REMOVED (No longer used by Grid Search)
...
# -----------------------------------------------------------------
# --- STAGE 2: HYPERPARAMETER TUNING (Exhaustive Grid Search)
# -----------------------------------------------------------------
print("\n" + "=" * 50)
print(f"STAGE 2: Exhaustive Grid Search Hyperparameter Tuning")
print("=" * 50)
...
# This is a manual definition of the grid, replacing Optuna's `suggest_` calls.
param_grid = {
 "n_features_to_select": [100, 200, 300, 400, 500], # 9 values
 "scale_pos_weight_factor": [0.2],                          # 3 values
 "learning_rate": [0.01, 0.05, 0.1, 0.2, 0.3],                         # 5 values
 "max_depth": [1, 2, 3, 4, 5, 6, 7, 8],                                         # 5 values
 "colsample_bytree": [0.3, 0.6, 0.9],                                  # 3 values
 "reg_lambda": [1.0, 50.0, 100.0],                               # 4 values
}

# param_grid = {
#     "n_features_to_select": [100], # 9 values
#     "scale_pos_weight_factor": [0.2],                          # 3 values
#     "learning_rate": [0.01],                         # 5 values
#     "max_depth": [1, 2, 3, 4, 5, 6, 7, 8],                                         # 5 values
#     "colsample_bytree": [0.3],                                  # 3 values
#     "reg_lambda": [1.0],                               # 4 values
# }

# --- 2. Calculate total combinations and warn user ---
total_combinations = 1
for key in param_grid:
    total_combinations *= len(param_grid[key])

print("\n" + "!" * 60)
print("!  WARNING: EXHAUSTIVE GRID SEARCH INITIATED  !")
print("!" * 60)
print(f"   Total parameter combinations to test: {total_combinations}")
print(f"   (Each combination runs RFE + {N_SPLITS_CV}-Fold Cross-Validation)")
print("   This process will be EXTREMELY slow and may take days or weeks.")
print("!" * 60 + "\n")

# --- 3. Initialize trackers ---
best_aucpr = -1.0
best_params_combo = {}
best_iteration_found = 100 # Default
all_trials_results = []
trial_counter = 0

# --- 4. Manually iterate through the grid (replaces study.optimize) ---
# Use itertools.product to get all combinations cleanly
grid_combinations = list(itertools.product(
    param_grid["n_features_to_select"],
    param_grid["scale_pos_weight_factor"],
...
print(f"Starting grid search... 0 / {total_combinations} completed.")

for combo in grid_combinations:
    trial_counter += 1
    trial_start_time = time.time()
    
    # --- A. Unpack parameters for this combination ---
    n_features = combo[0]
    scale_factor = combo[1]
    ...
    current_params = {
        ...
        "reg_lambda": combo[5],
        "random_state": RANDOM_STATE,
        **GRIDSEARCH_DEVICE_PARAMS # <-- CUDA MODIFICATION: Unpack GPU (or CPU) params
    }
    ...
    try:
        # --- B. RFE Step 2 (inside trial) ---
        # (This logic is copied directly from the original Optuna objective)
        rfe_step2_estimator = xgb.XGBClassifier(
            n_estimators=50, # Lighter model for RFE
            max_depth=5,
            learning_rate=0.1,
            random_state=RANDOM_STATE,
            scale_pos_weight=scale_pos_weight,
        ...
        # --- C. Model Training (inside trial) using 10-fold Group CV ---
        # (This logic is copied directly from the original Optuna objective)
        ...
        cv_results_df = xgb.cv(
            params=current_params,    # <-- This now contains GPU/CPU params
            dtrain=dtrain,
        ...
        current_aucpr = float(cv_results_df["test-aucpr-mean"].iloc[-1])
            
        # 5. Save the best number of iterations
        current_iteration = len(cv_results_df)

    except Exception as e:
        print(f"   WARNING: Trial {trial_counter} failed with error: {e}")
        trial_status = "FAIL"
        error_message = str(e)
        current_aucpr = 0.0 # Give it a score of 0 so it's not chosen
    
    trial_duration = time.time() - trial_start_time

    # --- D. Store results for this combination ---
    trial_result = {
        "trial": trial_counter,
        "status": trial_status,
        "aucpr": current_aucpr,
        ...
        "n_features_to_select": n_features,
        "scale_pos_weight_factor": scale_factor,
        **current_params # Add all other params
    }
    all_trials_results.append(trial_result)

    # --- E. Check if this is the new best ---
    if current_aucpr > best_aucpr:
        best_aucpr = current_aucpr
        best_iteration_found = current_iteration
        # Store the full set of parameters that led to this
        best_params_combo = {
            "n_features_to_select": n_features,
            "scale_pos_weight_factor": scale_factor,
            "learning_rate": combo[2],
        ...
        print(f"   >>> NEW BEST found at trial {trial_counter} / {total_combinations}")
        print(f"   >>> New Best Mean CV AUCPR: {best_aucpr:.6f}")
        print(f"   >>> Params: {best_params_combo}")
    
    if trial_counter % 10 == 0 or trial_counter == total_combinations:
         print(f"   Grid search progress: {trial_counter} / {total_combinations} completed. (Last best AUCPR: {best_aucpr:.6f})")


print(f"\nGrid search complete in {(time.time() - start_grid_time) / 3600:.2f} hours.")
print(f"   Best Mean CV AUCPR found: {best_aucpr:.6f}")

# --- 5. Extract best parameters and save all results ---
print(f"   3. Saving best parameters to {BEST_PARAMS_PATH}...")

# Extract the best parameters found
best_n_features = best_params_combo.get("n_features_to_select", 100) # Default to 100
best_factor = best_params_combo.get("scale_pos_weight_factor", 1.0)
best_iteration = best_iteration_found

# Create the final_params dict for XGBoost (Stage 3)
# Remove the non-XGBoost params from the combo
final_params = best_params_combo.copy()
final_params.pop("n_features_to_select", None)
final_params.pop("scale_pos_weight_factor", None)

# Add the calculated scale_pos_weight
...
print(f"   Best n_features_to_select found: {best_n_features}")
...
# Convert list of dicts to Polars DataFrame and save
try:
    trials_pl_df = pl.DataFrame(all_trials_results)
    trials_pl_df.write_csv(TUNING_TRIALS_PATH)
except Exception as e:
    print(f"   WARNING: Could not save trials CSV, falling back to pandas.")
    print(f"   Error: {e}")
    try:
        pd.DataFrame(all_trials_results).to_csv(TUNING_TRIALS_PATH, index=False)
    except Exception as e2:
        print(f"   ERROR: Could not save trials CSV with pandas either: {e2}")


print("=" * 50)
print("COMPLETED: STAGE 2: Exhaustive Grid Search")
print("=" * 50 + "\n")
# -----------------------------------------------------------------
# --- END: STAGE 2
# -----------------------------------------------------------------
