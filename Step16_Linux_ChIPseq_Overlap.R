...
setwd("/haynes-lab/Kim_SH/Projects/NIH R21/Codes/src")
...
# Optional fallback genome (used only if online UCSC fetch fails)
...
# Replace with the actual path to your input BED file
input_bed_file <- "../public/ChIPseq/SHK_2024_06_11_MCF7_DBN021_1_0_ug_mL_Rep2 Filtered_trim.bed"

# Replace with the desired name for the output inverse BED file
output_inverse_bed_file <- "../public/ChIPseq/SHK_2024_06_11_MCF7_DBN021_1_0_ug_mL_Rep2_inverse_output.bed"

# --- Step 2: Obtain hg38 chromosome information (Seqinfo) ---
# Prefer online fetch from UCSC; if that fails, fall back to BSgenome.
cat("Obtaining hg38 chromosome sizes/Seqinfo...\n")

get_hg38_seqinfo <- function() {
  # Try UCSC first (online)
  si <- tryCatch({
    GenomeInfoDb::getChromInfoFromUCSC(
      "hg38",
      assembled.molecules.only = TRUE,  # only primary assembled molecules
      as.Seqinfo = TRUE                 # directly return a Seqinfo
    )
  }, error = function(e) {
    message("Online fetch via UCSC failed: ", conditionMessage(e))
    NULL
  })
  
  # Fallback to BSgenome if UCSC failed
  if (is.null(si)) {
    if (requireNamespace("BSgenome.Hsapiens.UCSC.hg38", quietly = TRUE)) {
      library(BSgenome.Hsapiens.UCSC.hg38)
      si <- seqinfo(BSgenome.Hsapiens.UCSC.hg38)
    } else {
      stop("Could not retrieve hg38 chromosome info from UCSC, and BSgenome.Hsapiens.UCSC.hg38 is not installed.")
    }
  }
  
  # Keep only standard chromosomes (chr1-22, chrX, chrY), drop chrM, alts, etc.
  si_std <- GenomeInfoDb::keepStandardChromosomes(si, pruning.mode = "coarse")
  
  # *** UPDATE 1: Explicitly drop chrM to only keep chr1-22, X, Y as requested ***
  si_std <- GenomeInfoDb::dropSeqlevels(si_std, "chrM", pruning.mode = "coarse")
  
  # Ensure genome tag is set
  genome(si_std) <- "hg38"
  si_std
}

si_hg38 <- get_hg38_seqinfo()
standard_chroms <- GenomeInfoDb::standardChromosomes(si_hg38)
cat("Using chromosomes:", paste(standard_chroms, collapse = ", "), "\n")

# --- Step 3: Build a GRanges covering the entire hg38 genome (standard chroms only) ---
seqlens <- seqlengths(si_hg38)
hg38_genome <- GRanges(
  seqnames = names(seqlens),
  ranges = IRanges(start = 1, end = seqlens)
)
seqlengths(hg38_genome) <- seqlens
genome(hg38_genome) <- "hg38"
hg38_genome <- sort(hg38_genome)

# --- Step 4: Read your input BED file ---
cat("Reading input BED file:", input_bed_file, "\n")
if (!file.exists(input_bed_file)) {
  stop("Input BED file not found at the specified path: ", input_bed_file)
}

input_regions <- tryCatch({
  import(input_bed_file, format = "bed")
}, error = function(e) {
  stop("Failed to import the BED file. Please ensure it is a valid BED format.\nError: ", e$message)
})

# Harmonize seqlevels style to UCSC and drop non-standard chromosomes
suppressWarnings({
  GenomeInfoDb::seqlevelsStyle(input_regions) <- "UCSC"
})

# Keep only seqlevels in common with our genome
common_levels <- intersect(seqlevels(input_regions), seqlevels(hg38_genome))
if (length(common_levels) == 0L) {
  stop("No overlapping seqlevels between input BED and hg38 reference. ",
       "Check if your BED uses UCSC-style chromosome names (e.g., 'chr1').")
}

input_regions <- keepSeqlevels(input_regions, common_levels, pruning.mode = "coarse")

# Set seqinfo to hg38 Seqinfo and trim ranges to chromosome boundaries
seqinfo(input_regions) <- si_hg38[common_levels]
genome(input_regions) <- "hg38"
input_regions <- trim(input_regions)

# Remove strand information and reduce overlapping/adjacent ranges
strand(input_regions) <- "*"
input_regions <- reduce(input_regions, ignore.strand = TRUE)
input_regions <- sort(input_regions)

# --- Step 5: Calculate the complement (inverse) ---
cat("Calculating the inverse regions...\n")
inverse_regions <- setdiff(hg38_genome, input_regions, ignore.strand = TRUE)
inverse_regions <- sort(inverse_regions)

# --- Step 6: Export the inverse regions to a new 3-column BED file ---
# *** UPDATE 2: Changed export method to produce a 3-column BED file ***
cat("Exporting inverse regions to:", output_inverse_bed_file, "\n")

# Convert GRanges to a data.frame for custom export
inverse_df <- as.data.frame(inverse_regions)

# Create a new data.frame with just the required 3 columns.
# Note: BED format is 0-based for start, so subtract 1 from the start coordinate.
output_df <- data.frame(
  chrom = inverse_df$seqnames,
  start = inverse_df$start - 1,
  end = inverse_df$end
)

# Write to a tab-separated file without headers, quotes, or row names
write.table(
  output_df,
  file = output_inverse_bed_file,
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
  col.names = FALSE
...
cat("Successfully generated the inverse BED file.\n")
...
# Install BSgenome package for hg38
...
# Define file paths
single_bed_file <- "../public/Public Dataset/ChIPseq/GSE96363 ENCFF669NUD hg38 MCF7 H3K27me3 ChIPseq Filtered_trim.bed"

# Assign a name for the single bed file dataset for use in console output
single_bed_file_name <- "H3K27me3"
...
permutation_results <- bplapply(bed_files_peaks_list, function(public_peaks) {
  ...
  pt <- permTest(A = single_bed_file_peaks, B = public_peaks, ntimes = 1000,
                 ...
                 genome = BSgenome.Hsapiens.UCSC.hg38, # <--- Corrected version
                 verbose = FALSE)
  return(pt)
})
