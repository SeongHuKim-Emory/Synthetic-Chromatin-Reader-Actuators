...
BASE_DIR = os.path.join(os.path.dirname(__file__), '..', '..', 'public', 'Multiomics', 'Step19_ML_UpDEG_Prediction_SRA')
...
def get_gene_info_batch(identifiers, batch_size=200, max_retries=5, base_sleep=5):
    """
    Fetches gene details for a list of identifiers using efficient batch requests to the Ensembl API.
    Handles both Ensembl IDs and Gene Symbols and processes them in chunks.
    Adds retry logic with exponential backoff to handle temporary Ensembl outages or timeouts.
    """
    gene_info_map = {}
    ...
    headers = {
        ...
        # User-Agent is recommended by Ensembl so they can identify heavy clients if needed
        "User-Agent": "HaynesLab-GeneHancerPipeline/1.0 (contact: your_email@institution.edu)"
    }

    def run_with_retries(url, payload_key, batch):
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        decoded_symbols = run_with_retries(server + "/lookup/symbol/homo_sapiens", "symbols", batch)
    ...
    # Process Ensembl IDs in batches
    for i in range(0, len(ensembl_ids), batch_size):
        batch = ensembl_ids[i:i + batch_size]
        decoded_ids = run_with_retries(server + "/lookup/id", "ids", batch)
        if not decoded_ids:
            continue
